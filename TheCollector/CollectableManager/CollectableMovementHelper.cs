...
public class CollectableMovementHelper
{
    
    public static unsafe bool IsFlyingSupported => Svc.ClientState.TerritoryType != 0 &&
                                                   Svc.Data.GetExcelSheet<TerritoryType>()
                                                      .GetRowOrDefault(Svc.ClientState.TerritoryType)
                                                      ?.TerritoryIntendedUse.RowId is 1 or 49 or 47 &&
                                                   PlayerState.Instance()->IsAetherCurrentZoneComplete(
    ...
    internal static bool Move(
        IGameObject? gameObject, float tollerance = 0.25f, float lastPointTollerance = 0.25f, bool fly = false,
        bool useMesh = true)
    {
        if (gameObject == null)
            return true;
    ...
    internal static unsafe bool Move(
        ...
        bool useMesh = true)
    {
        ...
        if (position == Vector3.Zero ||
            Vector3.Distance(position, Player.Position) - (useMesh ? 0 : 1) <= lastPointTollerance)
        {
            if (position != Vector3.Zero)
                VNavmesh_IPCSubscriber.Path_Stop();
        ...
        if (PlayerHelper.IsMoving && !Player.Object.Struct()->Character.InCombat &&
            Vector3.Distance(Player.Object.Position, position) >= 10)
        {
            //sprint
            if (ActionManager.Instance()->GetActionStatus(ActionType.GeneralAction, 4) == 0 &&
                ActionManager.Instance()->QueuedActionId != 4 && !PlayerHelper.IsCasting)
                ActionManager.Instance()->UseAction(ActionType.GeneralAction, 4);

            //peloton
            if (ActionManager.Instance()->GetActionStatus(ActionType.Action, 7557) == 0 &&
                ActionManager.Instance()->QueuedActionId != 7557 && !PlayerHelper.IsCasting &&
                !Player.Object.StatusList.Any(x => x.StatusId == 1199))
        ...
        if (!PlayerHelper.IsReady || !VNavmesh_IPCSubscriber.Nav_IsReady() ||
            VNavmesh_IPCSubscriber.SimpleMove_PathfindInProgress() || VNavmesh_IPCSubscriber.Path_NumWaypoints() > 0)
            return false;
